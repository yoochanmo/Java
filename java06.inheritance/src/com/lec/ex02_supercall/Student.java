...
/*
 	...
 	부모객체도 예외는 아니다. 그렇다면 부모객체를 생성하기
 	위해서는 자식생성자에서 부모생성자가 호출되어야한다.
 	자식생성자에서 부모생성자를 호출할 경우에는 자식생성자의 맨 처음
 	(첫 줄)에 선언되고 호출되어야 한다.
 	...
 	super(매개값...)가 생략되면 컴파일러는 부모의 기본생성자가 super()가 자동으로 추가되기
 	때문에 부모클래스에 기본생성자가 반드시 존재해야한다.
