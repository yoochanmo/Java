...
/*
 	...
 	1 . DAO(Data Access Object)
 		...
 		DB에 접근을 하기 위한 로직과 비지니스(select,delect....)로직을
 		분리하는 개념이다.
 		
 		DAO는 DB에 접근해서 데이터를 조회하거나 조작하는 기능을 전담하도록
 		만든 객체를 말한다. 사용자는 자신이 필요한 Interface를 DAO에 전달하고
 		DAO는 이 인터페이스를 구현한 객체를 사용자가 편리하게 사용할 수 있도록
 		변환해 준다.
 		
 		DB에 접근은 DAO가 전담하도록하고 데이터베이스 엑세스를 DAO에서만 하게 되면
 		다수의 원격호출을 통한 오버헤드를 DTO(VO)를 통해 줄일 수 있고 다수의 DB호출로
 		DB서버에 부담을 줄일 수 있게 한다.
 		
 		
 	2 . DTO(Data Transfer Object, VO Value Object) 
 	
 		DTO는 데이터를 교환하는 즉, 데이터를 저장하는 그릇으로 이해하면 좋다.
 		일반적으로 DTO는 로직을 가지고 있지않고 데이터의 속성(필드)과 그 속성에
 		접근을 하기 위한 getter/setter메서드만 가진 클래스를 말한다.
 	...
 	3 . board 프로그램
 		...
 		6) BoardFactory			: 게시글을 생성 클래스(Singleton클래스)
